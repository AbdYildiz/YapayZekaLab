...
train = pd.read_csv("telefon_fiyat_degisimi.csv")
# test = pd.read_csv("test.csv")

label_encoder = LabelEncoder().fit(train.price_range)
labels = label_encoder.transform(train.price_range)
classes = list(label_encoder.classes_)

train = train.drop(["price_range"], axis=1)
train = train.drop(["blue"], axis=1)
train = train.drop(["fc"], axis=1)
train = train.drop(["int_memory"], axis=1)
train = train.drop(["ram"], axis=1)
train = train.drop(["wifi"], axis=1)
nb_features = 15
...
x_train = np.array(x_train).reshape(1600,15,1)
x_valid = np.array(x_valid).reshape(400,15,1)
